...
int main(int argc, char **argv) {
  ...
  if (value != nullptr && std::strcmp(value, "FLAT") == 0) {
    std::cerr << "Running test flat mode\n";
  } else {
    std::cerr << "synergy test error: this test can only be done by using FLAT mode\n";
    return -1;
  }
  // pick a root device
  ...
  synergy::utils::check_core_freq(q.get_synergy_device(), freq, 1000); // be sure that the frequency is set

  std::vector<int> a(N * N, 1);
  std::vector<int> b(N * N, 1);
  std::vector<int> c(N * N, 0);

  auto e1 = matmul(q, a, b, c, N);
  e1.wait();
  std::cout << "Target frequency: " << freq << " MHz\n";
  checkConsume(q, e1);
